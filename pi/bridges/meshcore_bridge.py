...
# ---------------------------------------------------------------------------
#  Icon set — white silhouette icons from ATAK default set
# ---------------------------------------------------------------------------

ICONSET_UID = "34ae1613-9645-4222-a9d2-e5f243dea2865"

# MeshCore node type → icon mapping
MESHCORE_ICONS = {
    0: f"{ICONSET_UID}/Military/radar.png",        # Unknown → radar
    1: f"{ICONSET_UID}/Military/radar.png",        # Repeater → radar tower
    2: f"{ICONSET_UID}/Military/soldier.png",      # Chat Node → person
    3: f"{ICONSET_UID}/Buildings/tower.png",       # Room Server → tower
}

WARDRIVING_ICON = f"{ICONSET_UID}/Military/SIGINT.png"
...
def make_status_cot(
    ...
    SubElement(detail, "usericon", {"iconsetpath": MESHCORE_ICONS.get(1, MESHCORE_ICONS[0])})
...
class MeshCoreBridge:
    ...
    def _make_advert_cot(
        ...
        # Icon from white silhouette set based on node type
        adv_type_num = {"Unknown": 0, "Repeater": 1, "Chat Node": 2, "Room Server": 3}.get(node_type, 0)
        icon_path = MESHCORE_ICONS.get(adv_type_num, MESHCORE_ICONS[0])
    ...
    def _handle_wardriving_point(self, sender: str, lat: float, lon: float, power: str, snr: float | None):
        ...
        SubElement(detail, "usericon", {"iconsetpath": WARDRIVING_ICON})
